...
class AnimationPipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        audio_fea_final = self.prepare_training_audio(audio_signal, video_length=video_length, fps=[fps, ] * audio_signal.shape[0], start_idx=[0, ] * audio_signal.shape[0], frame_stride=[1, ] * audio_signal.shape[0])
        ...
        for i, t in tqdm(enumerate(timesteps), total=len(timesteps), disable=(rank != 0)):
            ...
            offset_context_queue = list(context_scheduler(
                0, num_inference_steps, latents.shape[2] + offset, context_frames, context_stride, 0
    ...
    def prepare_training_audio(self, audio_signal, video_length, 
        frame_stride = [1],
        start_idx = [0],
        fps = [30]):
        # print(audio_signal.shape)
        audio_fea_finals = []
        if not isinstance(frame_stride, list):
            frame_stride = frame_stride.detach().numpy().tolist()
        if not isinstance(start_idx, list):
            start_idx = start_idx.detach().numpy().tolist()
        if not isinstance(fps, list):
            fps = fps.detach().numpy().tolist()
        # print('file audio_signal is', audio_signal)
        for i in range(audio_signal.shape[0]):
            whisper_feature = self.audio_guider.audio2feat(audio_signal[i])
            whisper_chunks = self.audio_guider.feature2chunks(feature_array=whisper_feature, fps=fps[i])
            ...
            clip_idx = list(range(start_idx[i], start_idx[i] + video_length * frame_stride[i], frame_stride[i]))
            audio_fea_finals.append(audio_fea_final[:, clip_idx])
        return torch.cat(audio_fea_finals)
    ...
    def train(
        ...
        audio_latents = self.prepare_training_audio(audio_signal, video_length, 
