...
def crop_and_resize_tensor(
    ...
    # print("int(top):int(bottom), int(left):int(right) is", int(top), int(bottom), int(left), int(right))
    # Image.fromarray(rearrange(frame_cropped, "b c h w -> b h w c")[0].numpy().astype('uint8')).save("after_crop.png")
    ...
    # Image.fromarray(rearrange(frame_resized, "b c h w -> b h w c")[0].numpy().astype('uint8')).save("after_resize.png")
...
def get_condition(control:torch.Tensor, 
    ...
    # DEBUG
    # Image.fromarray(control[0].astype("uint8")).save("control.png")
    # Image.fromarray(origin_video.astype("uint8")).save("origin_video.png")
    _, __, source_landmark = dwpose_model.dwpose_model(origin_video, output_type='np', image_resolution=H, get_mark=True)
    source_landmark = source_landmark["faces_all"][0] * 512.
    control_landmarks = []
    control_results = []
    for item in control:
        _, __, item_land = dwpose_model.dwpose_model(item, output_type='np', image_resolution=H, get_mark=True)
        item_res = dwpose_model(item)[control_aux_type]
        control_landmarks.append(item_land["faces_all"][0] * 512.)
    ...
    for item in control_results:
        aligned_img = cv2.warpAffine(item, transform_matrix, (H, W)) if switch_control_to_source else item
        control_aligns.append(aligned_img)
    return np.array(control_aligns), control
