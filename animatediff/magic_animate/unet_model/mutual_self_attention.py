...
from .attention import BasicTransformerBlock as _BasicTransformerBlock   # attention blocks from animatediff
...
class ReferenceAttentionControl():
    ...
    def register_reference_hooks(
        ...
        def hacked_basic_transformer_inner_forward(
            ...
            if self.only_cross_attention:
                ...
                if MODE == "read":
                    ...
                    # for x in self.bank:
                    #     print("self.bank shape is", x.shape)

                    # l = norm_hidden_states.shape[0] // 2
                    # hidden_states_uc = hidden_states.clone()
                    # encoder_hidden_states_t = torch.cat([norm_hidden_states] + self.bank, dim=1)
                    # for i in range(l):
                    #     hidden_states_ = torch.cat([norm_hidden_states[i:i+1], norm_hidden_states[i+l:i+l+1]])
                    #     encoder_hidden_states_ = torch.cat([encoder_hidden_states_t[i:i+1], encoder_hidden_states_t[i+l:i+l+1]])
                    #     hidden_states_ = self.attn1(hidden_states_, attention_mask=attention_mask, \
                    #         encoder_hidden_states=encoder_hidden_states_) + \
                    #         torch.cat([hidden_states[i:i+1], hidden_states[i+l:i+l+1]])

                    #     hidden_states_uc[i:i+1], hidden_states_uc[i+l:i+l+1] = hidden_states_[0:1], hidden_states_[1:2]
                    ...
                    if do_classifier_free_guidance:
                        ...
                        # l = norm_hidden_states[_uc_mask].shape[0]
                        # hidden_states_c_res = hidden_states[_uc_mask].clone()
                        # encoder_hidden_states_t = norm_hidden_states[_uc_mask]
                        # # print("norm_hidden_states[_uc_mask].shape is", norm_hidden_states[_uc_mask].shape)
                        # for i in range(l):
                        #     hidden_states_ = norm_hidden_states[_uc_mask][i:i+1]
                        #     encoder_hidden_states_ = encoder_hidden_states_t[i:i+1]
                        #     hidden_states_ = self.attn1(hidden_states_, attention_mask=attention_mask, \
                        #         encoder_hidden_states=encoder_hidden_states_) + \
                        #         hidden_states[_uc_mask][i:i+1]

                        #     hidden_states_c_res[i:i+1] = hidden_states_[0:1]
                        # hidden_states_c[_uc_mask] = hidden_states_c_res
                    ...
                    if self.attn2 is not None:
                        ...
                        # l = norm_hidden_states.shape[0] // 2
                        # hidden_states_res = hidden_states.clone()
                        # for i in range(l):
                        #     hidden_states_ = torch.cat([norm_hidden_states[i:i+1], norm_hidden_states[i+l:i+l+1]])
                        #     encoder_hidden_states_ = torch.cat([encoder_hidden_states[i:i+1], encoder_hidden_states[i+l:i+l+1]])
                        #     hidden_states_ = self.attn2(hidden_states_, attention_mask=attention_mask,  encoder_hidden_states=encoder_hidden_states_) + \
                        #         torch.cat([hidden_states[i:i+1], hidden_states[i+l:i+l+1]])
                        #     hidden_states_res[i:i+1], hidden_states_res[i+l:i+l+1] = hidden_states_[0:1], hidden_states_[1:2]
        ...
        def hacked_mid_forward(self, *args, **kwargs):
            ...
            print("arrive hacked_mid_forward")
