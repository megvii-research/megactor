...
from animatediff.utils.util import get_checkpoint
...
class MagicAnimate(torch.nn.Module):
    def __init__(self,
        ...
        *_, func_args = inspect.getargvalues(inspect.currentframe())
        ...
        # elif controlnet_type == '3d':
        #     assert controlnet_config != ''
        #     controlnet_config = OmegaConf.load(controlnet_config)
        #     self.controlnet = SparseControlNetModel.from_unet(self.unet, controlnet_additional_kwargs=controlnet_config.get("controlnet_additional_kwargs", {}))
        ...
        if hasattr(self.unet.condition_input_layer, "condition_atten_layer"):
            self.unet.condition_input_layer.condition_atten_layer.attn_temp.set_use_memory_efficient_attention_xformers(False)
        if not remove_referencenet:
        ...
        if "appearance_controlnet_motion_checkpoint_path" in config.keys() and config['appearance_controlnet_motion_checkpoint_path'] != "":
            ...
            # appearance_controlnet_motion_checkpoint_path = get_checkpoint(appearance_controlnet_motion_checkpoint_path)

            # buf = smart_load_from(appearance_controlnet_motion_checkpoint_path)
            # appearance_controlnet_motion_checkpoint_path = torch.load(
            #     buf, 
            #     map_location="cpu"
            # )
            ...
            # for (name1, param1), (name2, param2) in zip(self.controlnet.state_dict().items(), controlnet_state_dict.items()):
            #     param2 = param2.to(device="cpu", dtype=torch.float16)
            #     param1 = param1.to(device="cpu", dtype=torch.float16)
            #     if not torch.allclose(param1, param2, atol=1e-6):
            #         print(f'层 {name1} 的参数不同')
            #         print(f'self.controlnet is', param1)
            #         print(f'controlnet_state_dict is', param2)
        ...
        #     self.unet.conv_in.to(device=self.device, dtype=torch.float16)
    ...
    def infer(self, source_image, image_prompts, motion_sequence, random_seed, step, guidance_scale, context, size=(512, 768),froce_text_embedding_zero=False, ref_concat_image_noises_latents=None, do_classifier_free_guidance=True, add_noise_image_type="", ref_img_condition=None):
        prompt = n_prompt = ""
        step = int(step)
