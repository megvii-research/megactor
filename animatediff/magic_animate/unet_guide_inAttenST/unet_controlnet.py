...
class ConvInputLayer(nn.Module):
    ...
    def __init__(
        ...
        if self.use_temp:
            # tokenizer
            print("Condition ConvInputLayer using attention as temp!")
            ...
            # Feed-forward
            self.ff = FeedForward(dim=embed_dim, dim_out=conditioning_embedding_channels, dropout=0.0)
            ...
            # Only set ff to zero
            self.ff = zero_module(self.ff)

        
    def forward(self, x):
        x = x * 2. - 1.
        # b c f h w: b 3 f 512 512
    ...
    def _initialize_weights(self):
        ...
        torch.nn.init.zeros_(self.spatial_conv_out.weight)
...
class UNet3DConditionModel(ModelMixin, ConfigMixin):
    ...
    def __init__(
        ...
        self.condition_input_layer = ConvInputLayer(use_temp=use_temp_PoseGuider)
    ...
    def forward(
        ...
        sample = self.conv_in(sample) + self.condition_input_layer(pose_guide_conditions)
