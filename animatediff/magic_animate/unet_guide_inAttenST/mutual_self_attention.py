    ...
    def register_reference_hooks(
        ...
        def hacked_basic_transformer_inner_forward(
            ...
            if self.only_cross_attention:
                ...
                if MODE == "read":
                    if hidden_states.shape[0] != self.bank[0].shape[0]:
                        m = hidden_states.shape[0] // self.bank[0].shape[0]
                        self.bank = [rearrange(d.unsqueeze(1).repeat(1, m, 1, 1), "b t l c -> (b t) l c")[:hidden_states.shape[0]] for d in self.bank]
