...
class AnimationPipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        '''
        # class_null = torch.tensor([372] * batch_size, device=self._execution_device)
        # class_labels_input = torch.cat([class_null, class_null], 0) if do_classifier_free_guidance else class_null
        
        class_condition = torch.tensor([1000] * batch_size, device=self._execution_device)
        class_null = torch.tensor([1000] * batch_size, device=self._execution_device)
        class_labels_input = torch.cat([class_null, class_condition], 0) if do_classifier_free_guidance else class_null
        ...
        controlnet_cond_images = rearrange(controlnet_condition, "b f h w c -> (b f) c h w")
        ...
        for i, t in tqdm(enumerate(timesteps), total=len(timesteps), disable=(rank != 0)):
            ...
            for i in range(num_context_batches):
                ...
                # print('control_res_samples shape is', control_res_samples.shape)

                for j, k in enumerate(np.concatenate(np.array(context))):
                    controlnet_res_samples_cache_dict[k] = \
                        [sample[j:j + 1] for sample in control_res_samples]
                    # print("controlnet_res_samples_cache_dict[k] len", len(controlnet_res_samples_cache_dict[k]))
                    # print("controlnet_res_samples_cache_dict[k][0] shape", controlnet_res_samples_cache_dict[k][0].shape)
                    # print("controlnet_res_samples_cache_dict[k][1] shape", controlnet_res_samples_cache_dict[k][1].shape)
            ...
            for context in global_context[rank::world_size]:
                ...
                control_res = self.select_controlnet_res_samples(
                    controlnet_res_samples_cache_dict,
                    context,
                ...
                pred = self.unet(
                    ...
                    timestep=t, 
                    class_labels=class_labels_input, 
                    control_res_samples=control_res,
