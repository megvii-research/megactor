...
class ControlNetConditioningEmbedding(nn.Module):
    ...
    def forward(self, conditioning):
        embedding = self.pos_embed(conditioning)
        embedding = self.conv_out(embedding.transpose(1, 2)).transpose(1, 2)
...
class DiTControlNetModel(ModelMixin, ConfigMixin):
    ...
    def __init__(
        ...
        self.controlnet_blocks = nn.ModuleList(controlnet_blocks)
        self.controlnet_blocks = zero_module(self.controlnet_blocks)
    ...
    def from_dit(
        ...
        if load_weights_from_unet:
            ...
            controlnet.transformer_blocks[:num_layers].load_state_dict(dit_model.transformer_blocks[:num_layers].state_dict())
    ...
    def forward(
        ...
        if self.is_input_continuous:
            ...
            hidden_states += self.controlnet_cond_embedding(controlnet_cond)
        ...
        controlnet_results = []
        for i, (block, controlnet_block) in enumerate(zip(self.transformer_blocks, self.controlnet_blocks)):
            ...
            controlnet_result = controlnet_block(hidden_states.transpose(1, 2)).transpose(1, 2)
            controlnet_results.append(controlnet_result)
        return controlnet_results
