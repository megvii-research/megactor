...
class MagicAnimate(torch.nn.Module):
    def __init__(self,
        ...
        *_, func_args = inspect.getargvalues(inspect.currentframe())
        ...
        # DEBUG check params
        # params = [p.numel() if ("scale_shift" in n or "adaln_single" in n) else 0 for n, p in self.unet.named_parameters()]
        # print(f"### norm Module Parameters: {sum(params) / 1e6} M")

        # params = [p.numel() if "att" in n else 0 for n, p in self.unet.named_parameters()]
        # print(f"### attention Module Parameters: {sum(params) / 1e6} M")

        # params = [p.numel() if "caption" in n else 0 for n, p in self.unet.named_parameters()]
        # print(f"### Caption Module Parameters: {sum(params) / 1e6} M")

        # params = [p.numel() if "ff" in n else 0 for n, p in self.unet.named_parameters()]
        # print(f"### ff Module Parameters: {sum(params) / 1e6} M")

        # params = [p.numel() for n, p in self.unet.named_parameters()]
        # print(f"### ALL Module Parameters: {sum(params) / 1e6} M")
