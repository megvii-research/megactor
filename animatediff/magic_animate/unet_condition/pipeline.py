...
class AnimationPipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        for i, t in tqdm(enumerate(timesteps), total=len(timesteps), disable=(rank != 0)):
            ...
            for context in global_context[rank::world_size]:
                ...
                controlnet_cond_latents = rearrange(controlnet_cond_latents, "(b f) c h w -> b c f h w", b=1)
                controlnet_cond_latents = torch.cat([controlnet_cond_latents, controlnet_cond_latents])
                latent_model_input = torch.cat([latent_model_input, controlnet_cond_latents], dim=1)
    ...
    def train(
        ...
        controlnet_cond_latents = self.vae.encode(
                        control * 2. - 1.)['latent_dist'].mean * 0.18215
        controlnet_cond_latents = rearrange(controlnet_cond_latents, "(b f) c h w -> b c f h w", b=1)
        latents = torch.cat([latents, controlnet_cond_latents], dim=1)
