...
from animatediff.magic_animate.sparse_control.unet_controlnet import UNet3DConditionModel
from animatediff.magic_animate.sparse_control.controlnet import ControlNetModel
from animatediff.magic_animate.sparse_control.mutual_self_attention import ReferenceAttentionControl
...
class AnimationPipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        if prompt_embeddings is None:
            ...
            print("train use prompt_embeddings")
        ...
        # print('controlnet_condition is', controlnet_condition)
        # print('source_image is', source_image)
        ...
        for i, t in tqdm(enumerate(timesteps), total=len(timesteps), disable=(rank != 0)):
            ...
            for i in range(num_context_batches):
                ...
                controlnet_cond = self.vae.encode(controlnet_cond * 2. - 1.).latent_dist.sample() * 0.18215
                controlnet_cond = rearrange(controlnet_cond, "(b f) c h w -> b c f h w", f=context_frames)
                controlnet_cond_shape = list(controlnet_cond.shape)
                controlnet_cond_shape[1] = 1
                controlnet_conditioning_mask = torch.ones(controlnet_cond_shape, dtype=controlnet_cond.dtype).to(controlnet_latent_input.device)
                ...
                #     print('down_block_additional_residual shape is', x.shape)
                # for x in mid_block_res_sample:
                #     print('mid_block_additional_residual shape is', x.shape)
                # print('down_block_res_samples len is', len(down_block_res_samples))
                # print('mid_block_res_sample len is', len(mid_block_res_sample))
                # exit(0)
    ...
    def train(
        ...
        #     video_length, 0)
        controlnet_text_embeddings_c = text_embeddings
        ...
        # print('t is', t)
        # print('encoder_hidden_states shape is', controlnet_text_embeddings_c.shape)
        ...
        # print('controlnet_conditioning_mask shape is', controlnet_conditioning_mask.shape)
