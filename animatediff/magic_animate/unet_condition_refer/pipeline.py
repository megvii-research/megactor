...
class AnimationPipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        for i, t in tqdm(enumerate(timesteps), total=len(timesteps), disable=(rank != 0)):
            ...
            for context in global_context[rank::world_size]:
                ...
                controlnet_cond_latents = self.vae.encode(
                                controlnet_cond * 2. - 1.)['latent_dist'].mean * 0.18215
                controlnet_cond_latents = torch.cat([controlnet_cond_latents, controlnet_cond_latents]) if do_classifier_free_guidance else controlnet_cond_latents
                controlnet_cond_latents = torch.cat([ref_image_latents.repeat(controlnet_cond_latents.shape[0], 1, 1, 1), controlnet_cond_latents], dim=1)
                appearance_encoder(
                    controlnet_cond_latents,
                    t,
                    encoder_hidden_states=text_embeddings[:1].repeat(controlnet_cond_latents.shape[0], 1, 1),
                    return_dict=False,
                )
    ...
    def train(
        ...
        ref_image_latents = ref_image_latents.repeat(controlnet_condition.shape[1], 1 ,1 ,1)
        controlnet_condition = rearrange(controlnet_condition, "b f c h w -> (b f) c h w")
        ref_image_latents = torch.cat([ref_image_latents, controlnet_condition], dim=1)
        ...
        appearance_encoder(
            ref_image_latents,
            t,
            encoder_hidden_states=text_embeddings.repeat(ref_image_latents.shape[0], 1, 1),
            return_dict=False,
        )
