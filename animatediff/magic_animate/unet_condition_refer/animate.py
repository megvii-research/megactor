...
# from animatediff.magic_animate.sparse_control.unet import UNet3DConditionModel
...
class MagicAnimate(torch.nn.Module):
    def __init__(self,
        ...
        *_, func_args = inspect.getargvalues(inspect.currentframe())
        ...
        unet_tmp_weights = self.appearance_encoder.conv_in.weight.clone()
        self.appearance_encoder.conv_in = torch.nn.Conv2d(4 + 4, unet_tmp_weights.shape[0], kernel_size=3, padding=(1, 1))
        with torch.no_grad():
            self.appearance_encoder.conv_in.weight[:, :4] = unet_tmp_weights # original weights
            self.appearance_encoder.conv_in.weight[:, 4:] = unet_tmp_weights # original weights
